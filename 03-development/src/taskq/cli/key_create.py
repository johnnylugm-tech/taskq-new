...
[FR-03] Implements the key-creation sub-command. The CLI is invoked as

    python -m taskq.cli.key_create key create --scope <scope>

and the in-process callable is ``main(argv: list[str]) -> int``.

The plaintext token is generated locally, **hashed** via
``hash_api_key`` (sha256), persisted as ``key_hash`` (AC-3.2 / AC-3.4),
and printed to stdout **exactly once**. Nothing else — no logs, no
metrics, no error body — exposes the plaintext (NFR-04).
...
def main(argv: Sequence[str]) -> int:
    ...
    # 32 random bytes -> urlsafe-base64 ~ 43 chars; we then strip padding
    # to keep the printed line clean. >=16 chars as the test asserts.
    ...
    APIKeyRepository().create(scope=scope, key_hash=key_hash)
