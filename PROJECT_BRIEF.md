...
## Key Constraints
- **Technical**: Python 3.11; FastAPI ASGI app;具體啟動 entry point 由框架決定
...
| NFR-06 | `architecture_constraints` | mandatory `.importlinter`: layers contract **plus** a forbidden contract banning `sqlalchemy` outside the data-access layer (the `repository` role in NFR-06, concrete path by framework) |
...
| NFR-08 | `mutation_testing` | `features.mutation_testing: true`; score ≥ 70 over the business layer + the data-access layer (NFR-06 roles; concrete paths by framework) |
...
- High-risk modules requiring per-module TDD coverage (concrete paths by
  framework; the four **architectural roles** must all exist):
  - async subprocess runner (FR-08)
  - single auth/authz decision layer (FR-04)
  - DB transaction boundary context manager (FR-06)
  - v3 data-migration revision (FR-07)
