...
| Placeholder 規範 | 任何 `TBD` / `TODO` / `<placeholder>` / 規格未明示的標記在新一輪的 spec revision 中必須**保留為下一輪的待辦**,命名為 `NFR-99` 或 `FR-XX-deferred`(`XX` 為流水號);Agent A 不得 silent drop,視為「未交付」維度 |
...
- **語言**:Python 3.11
- **形態**:ASGI 服務;啟動 entry point 與管理 CLI 子命令(migrate / seed / healthcheck / key create)的具體路徑由框架決定,SPEC 不指定 module path
...
- 驗證規則:**request body 驗證**(非空 / ≤1000 字元 / 注入字元黑名單)違反 → **HTTP 422** + problem+json
- **名稱唯一性違反 → HTTP 409** + problem+json(uniqueness 為資料層約束,非 request body 驗證;對齊 §8 #8 與 HTTP Status Map 的 409 行為)
...
- 金鑰由框架決定的 CLI 介面產生(例如 `python -m <module> key create --scope <scope>`);明文**只在建立當下印出一次**
...
- 全部資料存取經由**資料存取層**(`repository` layer per NFR-06),**業務層不得直接持有 `Session`**;具體目錄與 module path 由框架決定
...
- **N+1 為失敗條件**:列表端點回應一次請求所發出的 SQL 陳述數必須是 **常數 ≤ 4**(與回傳筆數無關);採 SQLAlchemy event listener 在 **{1, 100, 1000, 10000}** 筆資料下計數,variance = 0;具體組成預期為 1 × count + 1 × 主查詢 + ≤ 2 × 顯式 eager-loaded children 預載
...
- 產出 SBOM 檔案(由框架決定具體路徑),含每個依賴的 `name` / `version` / `license` / `direct|transitive`
...
- **mutation score ≥ 70**
- 範圍限定於 **業務邏輯層(services)** 與 **資料存取層(repositories)** 兩個架構分層(對應 NFR-06 layers contract),並在 `harness_config.json` 註記限定理由(執行時間預算);具體目錄與檔名由框架決定
...
- 至少涵蓋:CRUD 全鏈、401/403/404/409/422/429/503 每個錯誤碼各一例(500 由 §8 #19 觸發,**不**強制每輪都出現,因為屬於「unexpected」類別)、migration 往返、rate limit 觸發與恢復、graceful drain
...
- 每個 API handler ≤ 40 行(業務邏輯必須下沉到業務邏輯層;對應 NFR-06 layers contract 的 `service` 角色),具體目錄由框架決定
...
### 5.1 環境變數(由框架決定的 config module 讀取;`.env.example` 完整宣告)
...
> **§6 (資料夾結構) 已移除**：由框架（coding agent / framework convention）自行決定最終目錄佈局。NFR-06（分層約束）仍保留，但具體路徑與檔名不再事先指定。
...
| `architecture` | code-review-graph | NFR-06 分層 + migrations(目錄由框架決定) |
| `secrets_scanning` | gitleaks | 框架預設門檻 100 |

**CRG 校準鐵律**:`crg_cohesion_healthy` 保持預設值,不得為了讓專案通過而調降。

**高風險模組**(由框架決定具體模組路徑,但四類功能**必須存在且需 per-module TDD 覆蓋**):
- async 子進程執行器(對應 FR-08;具體 module path 由框架決定)
- 認證授權判定的單一中介層(對應 FR-04;具體 module path 由框架決定)
- 資料庫交易邊界 context manager(對應 FR-06;具體 module path 由框架決定)
- v3 資料搬遷 migration(對應 FR-07;具體 migration 檔名由框架決定)

> **角色不變,路徑變**:SPEC 指定四個**架構角色**必須存在;具體 module path 與檔名由框架決定。
